...
/**
 * Servlet implementation class ProductBestSelectController
 */
@WebServlet("/bestProduct.pb")
public class ProductBestSelectController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ProductBestSelectController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		  ...
		  ArrayList<Product> list = new ProductService().selectBestProduct(pi);
		  
		  request.setAttribute("pi", pi);
		  request.setAttribute("list", list);
		  
		  
		  request.getRequestDispatcher("views/selectProduct/productBestSelectPage.jsp").forward(request, response);
	}
